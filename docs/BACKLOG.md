...
| 3 | 디자인 토큰/테마 | v1(색상) 완료 | 문서 전역 색상 토큰 + 노드 참조 + export(:root/var) (폰트·간격 연기) |
...
**v2 — 연기**: 폰트·간격 토큰. 색상과 같은 패턴(메타 토큰 + 노드 참조 + export 변수)으로 확장.

**완료 기준(v1)**: 토큰 하나를 바꾸면 이를 참조하는 노드들이 캔버스·export에서 일괄 반영된다(증거: 위 테스트).
