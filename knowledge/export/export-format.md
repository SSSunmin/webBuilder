...
type: Reference
title: MD Export 형식
description: generateMarkdown의 세 가지 모드(spec/code/both), 모든 모드 앞에 붙는 포터블 컨텍스트(서문+컴포넌트 범례), 명세서 트리 직렬화 방식, React 코드 생성 방식, 이벤트·override 출력 규칙. v2에서 디자인 토큰 → CSS 변수 출력 추가. v3(Stage A)에서 flex 컨테이너 래퍼 클래스·flow 자식 relative emit 추가. v4(Stage B1)에서 overrideDecls/overrideLines에 padding/margin 추가, overrideSpacingDecl(0-emit) 신규. v5(Stage B2)에서 overrideDecls에 background 추가, overrideBackgroundDecl(항상 emit·dangling→transparent) 신규. spec.ts overrideLines에 bgSummary 추가. v6(Stage C-3)에서 hasLayoutOverride에 ov.layout 추가. pushWrapperOverrideRules가 mode 인자를 제거하고 bp마다 resolveFlow/Grid(node,bp)로 모드를 재해석해 전체 decl(forceResets)을 래퍼 media 블록에 emit — flex↔grid 스왑 시 해당 모드의 full decl이 올바르게 출력됨. spec.ts hasLayoutOverride에도 ov.layout 추가(overrideLines bp-aware 요약에 모드 스왑 반영).
resource: src/export/index.ts, src/export/spec.ts, src/export/code.ts, src/export/legend.ts
tags: [export, markdown, spec, code, events, portable, legend, design-tokens, layout, flex, grid, breakpoint, override, spacing, background, layout-mode-swap]
...
#### `.pg-N-c` 래퍼 per-bp override emit — Stage C-2/C-3

`pushWrapperOverrideRules(acc, cls, node)` — `hasLayoutOverride(node, bp)` 가 true인 bp마다 `.pg-N-c { … }` 규칙을 `acc.tablet` / `acc.mobile`에 추가한다.

- **Stage C-3에서 `mode` 인자 제거**: bp마다 `resolveFlow(node, bp)` / `resolveGrid(node, bp)`로 모드를 재해석한다. flex↔grid 스왑이 있으면 해당 bp에서 반대 모드의 resolver가 non-null을 반환해 해당 모드의 full decl을 emit한다.
- `hasLayoutOverride`(code.ts·spec.ts 양쪽) 검사 대상에 `ov.layout`(C-3 추가) 포함.
- `flowDecls(flow, forceResets=true)` / `gridDecls(grid, forceResets=true)`: override 블록은 `gap: 0px` · `grid-template-rows: none` 등 기본값도 명시적으로 emit하여 base 규칙이 누출되지 않도록 한다.
- **flex→grid 스왑 시 `flex: 0 0 auto` punt**: base 기준 `inGrid=true`인 자식은 `baseDecls`에서 `flex: 0 0 auto`를 받지 못한다. 해당 bp에서 grid→flex 스왑 시 자식이 flex-item으로서 shrink될 수 있다(코드 주석에 명문화된 수용된 punt — 자식이 고정 width를 가지고 flex-wrap이 처리). flex→grid 스왑은 `flex: 0 0 auto`가 grid에서 inert하므로 무해하다.
