...
description: EditorShell이 제공하는 5-zone 레이아웃, DnD 흐름(절대배치·flex 자식 sortable reorder+DragOverlay, grid 자식은 reorder 불가), 스냅 시스템(flow 자식 스냅 스킵), 키보드 단축키. Stage C-2에서 per-bp 레이아웃 파라미터 오버라이드 추가(래퍼 클래스 .pg-N-c에 @media 규칙 emit; 레이아웃 모드는 여전히 base 고정). NodeView는 FlowNodeView(useSortable)/StaticNodeView(useDraggable) 디스패처 구조.
...
tags: [editor, dnd, snap, architecture, flex, grid, reorder, flow, sortable, dragoverlay, breakpoint, override, layout]
